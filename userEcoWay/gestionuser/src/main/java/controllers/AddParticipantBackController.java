...
public class AddParticipantBackController {
    ...
    public AddParticipantBackController() throws SQLException {
    ...
    private void handleAdd() {
        try {
            if (validateFields()) {
                ...
                showAlert("Succès", "Participant ajouté avec succès", Alert.AlertType.INFORMATION);
            }
        } catch (SQLException e) {
            showAlert("Erreur SQL", "Erreur lors de l'ajout: " + e.getMessage(), Alert.AlertType.ERROR);
            e.printStackTrace();
        } catch (NumberFormatException e) {
            showAlert("Erreur", "Veuillez vérifier les champs numériques", Alert.AlertType.ERROR);
        }
    }
    @FXML

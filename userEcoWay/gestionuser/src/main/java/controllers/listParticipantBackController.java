...
void showParticipants() throws IOException {
    ...
    @FXML
    void viewDetails(participant participant) {
        if (participant != null) {
            try {

                FXMLLoader loader = new FXMLLoader(getClass().getResource("/DetailParticipantBack.fxml"));
                Parent root = loader.load();
                ...
                Stage stage = new Stage();
                stage.setScene(new Scene(root));
                stage.setTitle("Détails de la participant");
                stage.show();

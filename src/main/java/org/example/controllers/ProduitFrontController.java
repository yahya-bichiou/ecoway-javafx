...
public class ProduitFrontController{
    ...
    @FXML private TextField searchField; // Champ pour la recherche

    private int currentPage = 1;
    private int itemsPerPage = 3; // Modifier la pagination pour afficher 3 produits par page
    private categorie selectedCategory;
    private String currentSearchKeyword = ""; // Pour stocker le terme de recherche actuel
    private Timeline searchDelayTimeline; // Pour retarder la recherche lors de la saisie
    ...
    private Button createCategoryButton(categorie cat) {
        ...
        btn.setOnAction(e -> {
            ...
            if (!searchField.getText().isEmpty()) {
                // Si une recherche est active, effectuer la recherche dans la nouvelle catégorie
                currentSearchKeyword = searchField.getText().trim();
                loadSearchResults();
            } else {
                // Sinon, charger tous les produits de la catégorie
    ...
    private void loadProducts() {
        ...
        try {
            ...
            // Si une recherche est active, utiliser les méthodes de recherche
    ...
    // Méthode pour gérer la recherche dynamique
    private void setupDynamicSearch() {
        // Initialiser la timeline pour retarder la recherche (pour éviter trop d'appels pendant la saisie)
        searchDelayTimeline = new Timeline(new KeyFrame(Duration.millis(300), event -> {
            performSearch();
        }));
        searchDelayTimeline.setCycleCount(1);

        // Ajouter l'écouteur pour le champ de recherche
        searchField.textProperty().addListener((observable, oldValue, newValue) -> {
            // Réinitialiser et redémarrer le délai à chaque frappe
            searchDelayTimeline.stop();
            searchDelayTimeline.play();
        });
    }

    // Nouvelle méthode pour exécuter la recherche
    ...
    // Méthode pour charger les résultats de recherche
    private void loadSearchResults() {
        ...
        try {
            ...
            if (selectedCategory != null) {
                produits = produitService.searchProductsByCategorie(
                        selectedCategory.getId(),
                        currentSearchKeyword,
                        currentPage,
                        itemsPerPage
                );
                totalItems = produitService.countSearchResultsByCategorie(
                        selectedCategory.getId(),
                        currentSearchKeyword
                );
            } else {
                produits = produitService.searchProducts(
                        currentSearchKeyword,
                        currentPage,
                        itemsPerPage
                );
                totalItems = produitService.countSearchResults(currentSearchKeyword);
            }

            // Si aucun produit n'est trouvé
            if (produits.isEmpty()) {
                String message = selectedCategory != null
                        ? "Aucun produit trouvé pour \"" + currentSearchKeyword + "\" dans la catégorie " + selectedCategory.getNom()
                        : "Aucun produit trouvé pour \"" + currentSearchKeyword + "\"";
                // Ne pas afficher d'alerte pour chaque recherche sans résultat (meilleure expérience utilisateur)
                // showAlert("Aucun résultat", message);

                // Au lieu d'afficher une alerte, montrer un message dans l'interface
                Label noResultsLabel = new Label(message);
                noResultsLabel.setStyle("-fx-font-size: 16px; -fx-text-fill: #757575; -fx-padding: 20;");
            ...
            showAlert("Erreur", "Impossible de rechercher les produits: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void initialize() {
        ...
        setupDynamicSearch(); // Configuration de la recherche dynamique
        loadProducts();    // Charge initialement les produits
    }
    ...
    private void updatePaginationInfo(int totalItems) {
        ...
        pageInfo.setText(String.format("Page %d/%d", currentPage, Math.max(1, totalPages))); // Afficher l'information de pagination

        // Désactiver les boutons si nécessaire
        previousPageBtn.setDisable(currentPage <= 1); // Désactiver "Précédent" si la page actuelle est la première
        nextPageBtn.setDisable(currentPage >= totalPages || totalPages == 0); // Désactiver "Suivant" si la page actuelle est la dernière
    }
    ...
    // Cette méthode était dans le code original mais ne semble plus être utilisée maintenant
    // avec la recherche dynamique
    @FXML
